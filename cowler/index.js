...
// const bloom = require('bloom-filter-x');
...
//             let title = $(v).find('a').eq(1).text();
//             let dsc = $(v).find('p').text();
//             let image = $(v).find('img').eq(0).attr('.src');
//             let url = $(v).find('a').eq(0).attr('href');
//             console.log(url)
//             if(bloom.add(url)){
//                 arr.push({
//                     cid:1,
...
//             connection.query(data_insert,[v.cid,v.title,v.dsc,v.image,v.url,v.create_time,v.content,1],(err,relults,fildes) => {
//                 if(err) throw err
//                 console.log('插入完成')
//             })
//         })
